...
description: "维护 ETSI-ASN1-Assistant V4/V4.1 — 华为LI X2/X3/HI1接口IRI解码Web工具的全生命周期操作: 加功能/修BUG/测试/文档/部署/GitHub推送"
version: 1.2.0
author: Hermes Agent
...
此项目代码 **严禁通过外网 LLM API 进行分析或修复**。所有操作必须纯本地完成：
- **修代码**: 仅用 `patch` / `write_file` 直接修改
- **查故障**: 仅用 `terminal`(本地命令) + 本地知识库检索
- **不要**: 将代码文件送入 LLM 上下文（不做 `read_file` 后让模型分析源码逻辑——除非切换至本地模型）
- **关于 codex exec**: 同样不可用（codex 会把代码传到 GitHub Copilot API）

> 如果必须借助 LLM 能力分析代码，先切换至本地模型再操作。
...
├── x_interface_decoder.py       # X接口日志分析(SSF/RVF/ZTLIG) V4.1: JSON栈匹配
├── templates/                   # HTML模板(活跃: upload.html/result.html; 备选: *_v4.html)
...
⑤ 写测试报告: docs/tests/*.md 或 docs/测试验证报告_*.md
⑥ 写变更日志: docs/changelog/*.md
⑦ 写知识库: ~/knowledge/telecom/lawful_interception/*.md
⑧ 版本号统一更新（发布时执行）:
   - grep -rn "V4\\\\.[0-9]" --include="*.py" --include="*.html" --include="*.md" src/ templates/ docs/ README.md | grep -v "asn1tools\\|venv\\|__pycache__\\|import\\|set_version\\|_version\\|deprecat" → 逐一更新至新版本号
   - 更新 `docs/VERSION_HISTORY.md` 在顶部插入新版本条目
   - 更新 `README.md` 标题版本号
   - 更新 `src/templates/upload.html` 的 navbar badge 和页脚版本
   - 更新 `src/templates/result.html`/`result1.html`/`result2.html` 的页头版本
   - 更新 `src/templates/x_interface.html` 报告生成版本注释
   - 更新各源码文件头部 `V4.x` 注释
   - 更新 `src/tests/test_all.py` 中的版本注释
   - 更新 `src/asn_decode_api_v4.py` 中的版本注释
...
| 功能 | 说明 | 文件 | 状态 |
|------|------|------|------|
| 🔒 安全加固 | `secure_filename` 防护路径遍历攻击 | `app_linux_v4.py` | ✅ 保留 |
| 🧩 ASN.1 错误隔离 | 逐套编译捕获异常，单套失败不影响整体 | `asn_spec_v4.py` | ✅ 保留 |
| 🔍 JSON 栈匹配 | ZTLIG2 嵌套 JSON 解析(替换简单正则) | `x_interface_decoder.py` | ✅ 保留 |
| 📋 模板回退 | ⚠️ `upload_v4.html`/`result_v4.html` 缺失导航栏和大文件支持，已回退使用 `upload.html`/`result.html` | `app_linux_v4.py` | 🔄 已回退 |
| 📦 上传限制 | ⚠️ `MAX_CONTENT_LENGTH=50MB` 阻断大文件(200MB)上传，已移除 | `app_linux_v4.py` | 🔄 已移除 |
...
- PCAP/IRI 上传页模板: `upload.html`（含 nav-tabs 导航，链接到 /x-interface）
- PCAP 结果页模板: `result.html`
- X接口日志页模板: `x_interface.html`（含返回 HI2 解码器的按钮 `← HI2 解码器`）
- **注意**: `upload_v4.html`/`result_v4.html` 是简化模板，**缺少导航栏**（无 X接口日志链接），已在 V4.1 修复中回退
...
### 跨分支 Cherry-pick: main vs V4 目录结构差异
- **场景**: `origin/main` 分支的 ETSI-ASN1-Assistant 文件在 `etsi-asn1-assistant/` 子目录下，而 `V4` 分支在同一路径下放文件（无子目录前缀）
- **根因**: `ops-monitoring` 仓库的 `main` 分支是多项目 monorepo（含 diameter-decoder/ztlig-tools/STCS 等），`V4` 分支仅含 ETSI-ASN1-Assistant 且文件在根目录
- **处理方式**: 不能直接 `git merge`（两个分支历史无关），需要：
  1. `git fetch --all` 后查看 `git log --oneline V4..origin/main -- etsi-asn1-assistant/` 确认变更范围
  2. 用 `git show <sha> -- etsi-asn1-assistant/<file>` 查看每个变更文件的具体 diff
  3. 手动用 `patch` 工具逐文件应用补丁（不要用 cherry-pick，路径前缀差异会导致冲突）
  4. 每个补丁应用后运行单元测试验证 `pytest src/tests/test_all.py -v`
  5. 本地提交后推送到 `origin/V4`
- **陷阱**: main 分支的模板文件（如 `upload_v4.html`）可能与 V4 分支功能不兼容，实际使用中 `upload_v4.html` 缺少导航栏（无 X接口日志链接），不能直接替换 `upload.html`
- **验证**: 应用补丁后启动服务 `venv/bin/python3 src/app_linux_v4.py`，curl 检查首页包含 `/x-interface` 链接和 `nav-tab` 元素
...
- 后端: 全量处理（无 size 限制，V4.1 曾尝试加 50MB 限制但阻断合法大文件上传，已移除）
