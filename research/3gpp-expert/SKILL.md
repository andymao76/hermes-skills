...
**语言偏好：** 用户使用简体中文交流。除非用户主动使用英文，否则全部用中文解释技术概念。术语首次出现时保留英文原文并加中文说明（如："NR RRC（新空口无线资源控制协议）"）。使用中文命名规范：基站在技术上下文中使用 "gNB" 而非全称。
...
- **asn1-codec** — ASN.1 BER/PER 编解码专家。3GPP RRC/NAS 信令使用 PER Aligned 编码，协议消息的定义文件（.asn1）解析和调试需要该 skill。
- **etsi-lawful-intercept** — ETSI 合法监听标准体系。TS 102 232-7（移动网络监听）与 3GPP TS 33.128 紧密关联。
- **wireshark-analysis** — Wireshark 网络流量分析。可用 `tshark -V -Y` 结合 ASN.1 解码器分析 RRC/NAS 信令消息。
- **self-learning** — 自动学习新技术。用法：`/learn 3GPP Release 20 study items`

## Response Patterns

**For BER/PER/ASN.1 encoding analysis questions (raw hex codec, TLV structure, TAG verification):**
Use `~/ber-tag-analyzer.py` for automated BER TLV structure traversal and TAG verification under 3 assumptions (2-byte/4-byte/6-byte). For definitive verification, use pyasn1 (`from pyasn1.codec.ber import decoder`). Read `references/ber-encoding.md` for encoding rules and MAP protocol specifics.

**Important: LLMs (Qwen/DeepSeek) are unreliable for BER decoding** — they hallucinate structural nodes and misinterpret fields. BER is deterministic bit manipulation; use tools.
...
**For SPEC series classification (21-55 series), SVC/IMS/VoLTE/VoNR priority mapping, or learning roadmap questions:**
Read `references/spec-series-classification.md`.

**For BER/ASN.1 encoding analysis, MAP protocol codec, or TAG verification questions:**
Read `references/ber-encoding.md`.
