...
> **工作/项目问题必须先查本地资源，无结果后再上网搜索。**

查询优先级顺序（从高到低）：
1. **本地知识库** (`~/knowledge/`) — 实战经验、排错记录、厂商文档
2. **技能库** (`~/.hermes/skills/`) — 流程性知识、工作流模板
3. **笔记和记忆** — 跨会话记忆 + 历史会话记录 (session_search)
4. **网上搜索** (web_search / web_extract) — 仅当前三者无结果时执行

这条规则位于 Authority Hierarchy 之上，在任何企业搜索之前执行。
...
This skill covers **enterprise internal search** (chat, email, wiki, task trackers).
For **public web technical research** — especially when Wikipedia lacks a product entry — see the companion reference file:
- `references/wikipedia-missing-product-search.md` — keyword variants, source-type prioritization, multi-round search patterns for legacy telecom/industrial equipment documentation.

For **Chinese e-commerce price research** (Taobao/JD product price comparison, second-hand market research):
- `references/chinese-ecommerce-price-research.md` — query decomposition patterns, search templates, data presentation format, and common pitfalls for cross-platform price research.
