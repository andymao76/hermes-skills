...
def enrich_result_with_link_artifacts(result, title):
    ...
    if not isinstance(img_url, str): img_url = ""
    if not isinstance(visit_url, str): visit_url = ""
    if not img_url and not visit_url:
        return result
    copy_block = "\n".join([
        "图片原始链接（完整复制整行）:", img_url, "",
        "编辑查看原始链接（完整复制整行）:", visit_url,
    ])
...
def main():
    ...
    input_group.add_argument('--markdown', help='Markdown content or "-" for stdin')
    input_group.add_argument('--markdown-file', help='Path to a local Markdown file')
    parser.add_argument('--cleanup-markdown-file', action='store_true', help='Force-delete file after processing')
    parser.add_argument('--theme', help='Theme name or JSON')
    ...
    markdown_content, resolved_markdown_file = resolve_markdown_input(
        markdown_arg=args.markdown, markdown_file_arg=args.markdown_file)
    cleanup_target = None
    ...
    payload = {"title": args.title, "markdown": markdown_content, "structure": structure, "source": "skill_all_mind_official"}
