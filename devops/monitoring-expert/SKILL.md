...
| Docker Monitoring Stack | `references/docker-prometheus-host-network-targets.md` | Prometheus + node_exporter + cadvisor Docker 部署避坑 |
| Hermes Health Exporter | `references/hermes-health-exporter.md` | 自定义 Prometheus exporter — 代理/Provider/系统健康检查 |
| Grafana Provisioning | `references/hermes-health-exporter.md` | 数据源 + 仪表盘自动预配 (provisioning YAML) |
| Grafana Dashboard API Import | `references/hermes-health-exporter.md` | 通过 HTTP API 导入 Dashboard JSON (POST /api/dashboards/db) |

## Custom Exporters

### Hermes Health Exporter (systemd-based Python Exporter)

For exposing Hermes-specific metrics (proxy health, provider connectivity, system services) to Prometheus, use a lightweight Python HTTP server running as a systemd user service.

**Location:** `scripts/hermes_health_exporter.py` — reusable template for custom exporters.

**Key features:**
- Listens on port 9800, exposes `/metrics` endpoint
- Runs checks on each scrape (30s cache), no external dependencies beyond Python stdlib
- Covers: Clash proxy (GUI/Mihomo/port/functional test), Provider APIs (DeepSeek/SiliconFlow), Hermes services (Gateway/Chrome CDP/MCP), system load/memory/disk
- Exposes a composite `hermes_up` metric (1=all healthy)
- Deployed as systemd user service: `hermes-health-exporter.service`

**Installation:**
```bash
# Copy the service file
cp hermes-health-exporter.service ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now hermes-health-exporter

# Verify metrics
curl http://127.0.0.1:9800/metrics | grep "^hermes_"
```
...
scrape_configs:
  - job_name: 'hermes_health'
    scrape_interval: 30s
    static_configs:
      - targets: ['172.18.0.1:9800']  # Docker bridge gateway → host
```

### Grafana Provisioning (Auto-Configure on Deploy)

Avoid manual datasource/dashboard setup by mounting provisioning YAML files:
...
│   └── dashboards/
│       ├── dashboards.yaml     # file provider declaration
│       └── node-exporter-full.json  # dashboard JSON (ID 1860)
├── docker-compose.yml
└── prometheus.yml
```
...
**Pitfalls:**
- Grafana runs as uid 472 inside container → provisioning files need `chmod 644` and dirs `chmod 755`
- After adding new dashboard JSONs, restart Grafana: `docker compose restart grafana`
