...
软件版本发放说明（如 `LISTENER V1.1.02_TMC_T23版本发放说明.md`）的分析方法论。用于在收到新版本后，快速判断是否修复了已知问题。

### 6 步对比法

| 步骤 | 分析维度 | 检查内容 |
|:----:|---------|---------|
| 1 | **新增功能** | 读取 §3.1 所列修复项，逐条比对是否匹配已知问题描述 |
| 2 | **涉及模块** | 检查升级模块（`§7 版本升级说明` 的 `flink-tmc`/`ztlig`/`ssf` 等）是否与根因涉及的模块一致 |
| 3 | **故障解决清单** | 查看 `§4 工程故障解决清单` 是否为空——为空则无针对现场报告故障的专项修复 |
| 4 | **根因层级匹配** | 判断修复是应用层（flink-tmc）、采集层（ZTLIG2/SSF）还是数据模型层（Kafka Schema）——字段缺失类问题通常是采集层缺陷，flink 后端无法补救 |
| 5 | **升级路径范围** | 检查升级涉及哪些模块——只升级 flink-tmc 不可能修复 ZTLIG2/SSF 数据管道问题 |
| 6 | **交叉验证** | 将分析结论写入已知问题清单，待下次版本升级时再评估 |

### 典型输出格式

```
## 版本对比结论：T23 未修复 WLAN IP 缺失问题

| 维度 | 修复内容 | 我们的问题 |
|------|---------|-----------|
| 修复目标 | 位置回填事件5丢失 | WlanUeLocalIp 字段缺失 |
| 涉及模块 | flink-tmc | ZTLIG2/SSF 采集层 |
| 修复类型 | 事件丢失→管道逻辑 | 字段缺失→数据模型 |
| 问题层级 | 应用层（Flink） | 采集层（Kafka 消息） |
| 故障清单 | 空（无专项修复） | — |
```

### Pitfalls

- **模块名混淆**：发放说明中的 `flink-tmc` 是位置回填模块，`TMC` 也是管理平台缩写，不要混淆。需区分 TMC（管理平台）、flink-tmc（数据回填进程）、TMC_TARGET_INFO（Kafka Topic）。
- **空故障清单 = 无现场专项修复**：§4 表格为空说明该版本仅包含研发自测修复，未针对现场报告的特定故障做修改。
- **数据管道断层**：字段缺失类问题（如 WLAN-ue-local-ip 未出现在 Kafka CDR 中）涉及 ZTLIG2 或 SSF 解析逻辑，纯后端升级无法修复。
- **EWS 子版本注意**：EWS 系列版本（如 `LISTENER EWS_V1.0.02.xx`）是大版本的不同分支，修复内容可能不同，需单独分析。

### 参考案例

详见 `references/ztlig-release-note-analysis.md`，含 WLAN IP 缺失问题的完整版本对比分析。
