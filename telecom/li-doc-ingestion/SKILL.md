...
| `li/Ericsson/` | SOAP API WSDL 包、1 口对接、curl 实操、状态码、踩坑记录、埃塞现场经验 |
...
### 3.7 Tar 包/压缩包学习模式（user: "学习 *.tar / *.tar.gz / *.7z"）
...
### 3.7 SOAP 样本学习模式（user: 提供原始 SOAP XML 请求/响应样本）
...
2. **API 直调方案（vision_analyze 不可用时的备用路线）**：
...
- `li/Ericsson/LIMS_Workflow_and_Maintenance.md` — 爱立信LI-IMS集成: ZTLIG工作流与运维（设控→IRI→语音→排障全链路；⚠️ 爱立信场景专属，非通用项目运维）
