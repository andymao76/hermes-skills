...
### 3.14 项目版本归档分析模式（user: 分享项目软件版本包 `A1-version.zip` 等）

当用户提供的是 **项目软件版本归档包**（如 A1 项目 `.zip`，内含多个以日期/版本号命名的发布目录 + 离线任务清单），而非厂商 LI 协议文档时：

1. **探索包结构** — 解压到临时目录，列出全部文件和目录。识别版本目录命名的规律（如 `20250102/`、`sudan20250222/`）。构建完整版本链
2. **识别关键文档** — 从以下维度选取子集读取：
   - **最早基线**：`sudani1031/` 等最早目录，提取初始组件版本
   - **关键里程碑**：标记有版本号跳跃的目录（如 T1B7→T1B9、T2B7P14→T2B7P19）
   - **最新版**：最新日期目录，提取当前状态
   - **离线任务**：读取单独的离线任务清单文档
3. **提取三个维度信息**：
   - **版本时间线**：每个版本的 TMC/DIA/Listener 版本号、上线日期、关键功能
   - **技术栈演变**：JDK/MySQL/Kafka/Redis/Hadoop/ES/HBase/Hive/Spark/Flink/Greenplum/JanusGraph/Ambari 等组件版本对比
   - **补丁关联**：ZTLIG 补丁与版本功能的对应关系
4. **产出多篇关联笔记** — 按以下分离结构写入知识库，用 wikilink 互相引用：
   - **时间线总览**：`versions/A1版本时间线总览.md` — 版本节点表 + 特性详述 + 升级路径建议
   - **技术栈演变**：`versions/A1技术栈与补丁分析.md` — 组件版本对比表 + 补丁分析 + 升级检查清单
   - **运维体系**：`versions/A1离线任务体系.md` — crontab 配置、脚本功能、版本新增追踪
5. **笔记格式规范** — 每篇必须包含：
   - YAML frontmatter（title/tags/created/source/status）
   - wikilink（`[[li/Sinovatio/projects/A1/versions/XXX]]`）连接到同级及其他关联文档
   - `## 关联文档` 章节指向同主题的兄弟笔记
   - 内容级别标注（如 TABLE/CHECKLIST/MERMAID 等格式标识）
6. **索引更新** — 写入后运行 `kb-index`（如未登录告知用户路径）

> ⚠️ 项目版本归档分析不是 LI 协议文档分析，不需要厂商目录分类，统一放入 `li/projects/<项目名>/versions/`。避免混入 `li/<vendor>/` 协议目录。

### 3.15 工作/学习材料整理模式（user: 分享英语学习/技术教程等非 LI 内容）
